...
def transform_my(dataset, scale_factor):
    path = './' + "ssb" +'/loader/'
    ip = f"{dataset}/SF_{scale_factor}"
    op = '../data/s%d_columnar/' % scale_factor
    with cd(path):
        os.system('mkdir -p %s' % op)
        os.system('./loader --lineorder %s/lineorder.tbl --ddate %s/date.tbl --customer %s/customer.tbl --supplier %s/supplier.tbl --part %s/part.tbl --datadir %s' % (ip, ip, ip, ip, ip, op))
