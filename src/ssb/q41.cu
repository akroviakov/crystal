...
template<QueryVariant QImpl>
__global__ void build_hashtable_p_Compiled(int *filter_col, int *dim_key, int num_tuples, int *hash_table, int num_slots, int batchSize) {
  auto body = [=] __device__ (int offset) {
...
template<QueryVariant QImpl>
__global__ void build_hashtable_c_Compiled(int* filter_col, int *dim_key, int* dim_val, int num_tuples, int *hash_table, int num_slots, int batchSize) {
  auto body = [=] __device__ (int offset) {
    if(filter_col[offset] == 1){
      int hash = HASH(dim_key[offset], num_slots, 0);
...
float runQuery(int* lo_orderdate, int* lo_custkey, int* lo_partkey, int* lo_suppkey, int* lo_revenue, int* lo_supplycost, int lo_len,
  ...
  if constexpr(QImpl == QueryVariant::Vector || QImpl == QueryVariant::VectorOpt){
    ...
    build_hashtable_c_Compiled<QImpl><<<gridSizeC, blockSizeC>>>(c_region, c_custkey, c_nation, c_len, ht_c, c_len, batchSize);
    build_hashtable_p_Compiled<QImpl><<<gridSizeP, blockSizeP>>>(p_mfgr, p_partkey, p_len, ht_p, p_len, batchSize);
