...
template<QueryVariant QImpl>
__global__ void build_hashtable_d_Compiled(int* filter_col, int *dim_key, int *dim_val, int num_tuples, int *hash_table, int num_slots, int val_min, int batchSize) {
  auto body = [=] __device__ (int offset) {
    if(filter_col[offset] == 199712){
...
float runQuery(int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_revenue, int lo_len,
  ...
  if constexpr(QImpl == QueryVariant::Vector || QImpl == QueryVariant::VectorOpt){
    ...
    build_hashtable_d_Compiled<QImpl><<<gridSizeD, blockSizeD>>>(d_yearmonthnum, d_datekey, d_year, d_len, ht_d, d_val_len, d_val_min, batchSize);
  }
