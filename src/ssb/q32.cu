...
float runQuery(int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_revenue, int lo_len,
  ...
  if constexpr(QImpl == QueryVariant::Vector || QImpl == QueryVariant::VectorOpt){
    ...
    build_hashtable_s_Compiled<QImpl><<<gridSizeS, blockSizeS>>>(s_nation, s_suppkey, s_city, s_len, ht_s, s_len, batchSize);
    build_hashtable_c_Compiled<QImpl><<<gridSizeC, blockSizeC>>>(c_nation, c_custkey, c_city, c_len, ht_c, c_len, batchSize);
